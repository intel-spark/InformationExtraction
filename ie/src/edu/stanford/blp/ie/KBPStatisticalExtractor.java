...
public class KBPStatisticalExtractor implements edu.stanford.nlp.ie.KBPRelationExtractor, Serializable {
  ...
  private static final Redwood.RedwoodChannels log = Redwood.channels(edu.stanford.nlp.ie.KBPStatisticalExtractor.class);
...
//      indicator(feats, "deppath_unlex", StringUtils.join(depparsePath.subList(1, depparsePath.size() - 1).stream().filter(x -> x.startsWith("-") || x.startsWith("<")), ""));
  ...
  private static void relationSpecificFeatures(KBPInput input, Sentence sentence, ClassicCounter<String> feats) {
    if (input.objectType.equals(edu.stanford.nlp.ie.KBPRelationExtractor.NERTag.NUMBER)) {
    ...
    if (
        (input.subjectType == edu.stanford.nlp.ie.KBPRelationExtractor.NERTag.PERSON && input.objectType.equals(edu.stanford.nlp.ie.KBPRelationExtractor.NERTag.ORGANIZATION)) ||
            (input.subjectType == edu.stanford.nlp.ie.KBPRelationExtractor.NERTag.ORGANIZATION && input.objectType.equals(edu.stanford.nlp.ie.KBPRelationExtractor.NERTag.PERSON))
        ) {
  ...
  public static void main(String[] args) throws IOException, ClassNotFoundException {
    ...
    ArgumentParser.fillOptions(edu.stanford.nlp.ie.KBPStatisticalExtractor.class, args);  // Fill command-line options

    // Load the test (or dev) data
    forceTrack("Test data");
    List<Pair<KBPInput, String>> testExamples = edu.stanford.nlp.ie.KBPRelationExtractor.readDataset(TEST_FILE);
    ...
    if (!IOUtils.existsInClasspathOrFileSystem(MODEL_FILE)) {
      ...
      // Save the classifier
      IOUtils.writeObjectToFile(new edu.stanford.nlp.ie.KBPStatisticalExtractor(classifier), MODEL_FILE);
    }

    // Read either a newly-trained or pre-trained model
    Object model = IOUtils.readObjectFromURLOrClasspathOrFileSystem(MODEL_FILE);
    edu.stanford.nlp.ie.KBPStatisticalExtractor classifier;
    if (model instanceof Classifier) {
      //noinspection unchecked
      classifier = new edu.stanford.nlp.ie.KBPStatisticalExtractor((Classifier<String, String>) model);
    } else {
      classifier = ((edu.stanford.nlp.ie.KBPStatisticalExtractor) model);
    }
