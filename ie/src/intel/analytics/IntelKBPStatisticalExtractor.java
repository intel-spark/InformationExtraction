...
public class IntelKBPStatisticalExtractor implements IntelKBPRelationExtractor, Serializable {
    ...
    private static final Redwood.RedwoodChannels log = Redwood.channels(IntelKBPStatisticalExtractor.class);
...
//      indicator(feats, "deppath_unlex", StringUtils.join(depparsePath.subList(1, depparsePath.size() - 1).stream().filter(x -> x.startsWith("-") || x.startsWith("<")), ""));
    ...
    private static void relationSpecificFeatures(KBPInput input, Sentence sentence, ClassicCounter<String> feats) {
        if (input.objectType.equals(IntelKBPRelationExtractor.NERTag.NUMBER)) {
        ...
        if (
                (input.subjectType == IntelKBPRelationExtractor.NERTag.PERSON && input.objectType.equals(IntelKBPRelationExtractor.NERTag.ORGANIZATION)) ||
                        (input.subjectType == IntelKBPRelationExtractor.NERTag.ORGANIZATION && input.objectType.equals(IntelKBPRelationExtractor.NERTag.PERSON))
                ) {
    ...
    public static void main(String[] args) throws IOException, ClassNotFoundException {
        ...
        IntelKBPStatisticalExtractor classifier;
        if (model instanceof Classifier) {
            //noinspection unchecked
            classifier = new IntelKBPStatisticalExtractor((Classifier<String, String>) model);
        } else {
            classifier = ((IntelKBPStatisticalExtractor) model);
        }
